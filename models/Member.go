...
func (m *Member) ldapLogin(account string, password string) (*Member, error) {
    ...
    searchRequest := ldap.NewSearchRequest(
        ...
        []string{"dn", "mail","sAMAccountName","cn","displayName","description","mobile","department","company"},
        nil,
    )
    ...
    if m.MemberId <= 0 {
        ...
        m.RealName = searchResult.Entries[0].GetAttributeValue("cn")
        m.Description = (searchResult.Entries[0].GetAttributeValue("company") + "-" + searchResult.Entries[0].GetAttributeValue("department") + "-" + searchResult.Entries[0].GetAttributeValue("description"))
        m.Phone = searchResult.Entries[0].GetAttributeValue("mobile")
        m.AuthMethod = "ldap"
...
func (m *Member) Add() error {
    ...
    if ok, err := regexp.MatchString(conf.RegexpAccount, m.Account); m.Account == "" || !ok || err != nil {
        return errors.New("账号只能由英文字母数字组成，且在2-50个字符")
    }
    if m.Email == "" {
...
//    m.Password = hash
//  ldap用户登录不计算password的hash值，从ldap验证，解决ldap用户修改密码后，旧密码仍能使用的bug
    if m.AuthMethod == "ldap" {
        m.Password = ""
    }else {
        m.Password = hash
    }
